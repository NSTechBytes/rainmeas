# Script to generate NSIS script with correct version
param(
    [string]$VersionFile,
    [string]$TemplateFile = "setup.template.nsi",
    [string]$OutputFile = "setup.nsi"
)

# Read version from VERSION file
# If VersionFile parameter is not provided, try multiple paths to find the VERSION file
if (-not $VersionFile) {
    $VersionFilePaths = @(
        "..\VERSION",  # From nsi-installer directory
        "VERSION",     # From current directory
        "..\..\VERSION" # From build directory
    )

    foreach ($Path in $VersionFilePaths) {
        if (Test-Path $Path) {
            $VersionFile = $Path
            break
        }
...
Set-Content -Path $OutputFile -Value $nsisContent
Write-Host "Generated NSIS script: $OutputFile"